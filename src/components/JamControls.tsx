...
interface Props {
  ...
  mode: JamMode;
  onLeave: () => void;
  onChangeMode: (mode: JamMode) => void;
}

export default function JamControls({ isHost, inviteCode, mode, onLeave, onChangeMode }: Props) {
  ...
  return (
    <div className="space-y-2">
      {/* Mode checkbox */}
      <label className={`flex items-center gap-2 px-1 ${isHost ? "cursor-pointer" : ""}`}>
        <input
          type="checkbox"
          checked={mode === "anyone"}
          disabled={!isHost}
          onChange={(e) => onChangeMode(e.target.checked ? "anyone" : "host_only")}
          className="h-3.5 w-3.5 rounded border-neutral-700 bg-neutral-800 text-orange-500 accent-orange-500 focus:ring-0 focus:ring-offset-0 disabled:opacity-50"
        />
